...
void USBHAL::setAddress(uint8_t address) {
    ...
    //mbed_error_printf(USBD_DEBUG_LEVEL,"setAddress cmd is  0x%02x, address is 0x%02x\r\n", cmd, address);
    g_address = cmd;
    g_ep0_state = USB_EP0_STAGE_ACKWAIT;
    set_addr = 1;
}
...
void USBHAL::remoteWakeup(void) {
	//mbed_error_printf("remoteWakeup\r\n");
}
...
void USBHAL::usbisr(void) {
    ...
    //mbed_error_printf("usbisr current_ep_num is 0x%02X \r\n", current_ep_num);

    /* handle Rx endpooint interrupt(EP0 has no RX isr) */
    int_stat = RDA_USB->INTRRX;
    //mbed_error_printf("rx interrupt INTRRX is 0x%04X , INTRRXEN is 0x%02x\r\n",
    //                  int_stat, RDA_USB->INTRRXEN);

    int_stat &= RDA_USB->INTRRXEN;
    //mbed_error_printf("rx interrupt INTRRXEN is 0x%04X \r\n", int_stat);
    ...
    if (int_stat & BIT0) {
        //mbed_error_printf("EP0 interrupt\r\n");
        uint16_t csr = 0;

        RDA_USB->EPIDX = 0;
        csr = RDA_USB->CSR0;
        //mbed_error_printf("csr is 0x%04X \r\n", csr);
        ...
        if (csr & USB_CSR0_P_SETUPEND) {
            //mbed_error_printf("EP0 SETUPEND\r\n");
        ...
        switch (g_ep0_state) {
            ...
            case USB_EP0_STAGE_RX: {
                if ((csr & USB_CSR0_RXPKTRDY) == 1) {
                    //mbed_error_printf("EP0 out\r\n");
                    EP0out();
                }
            }
                break;
            case USB_EP0_STAGE_ACKWAIT: {
                //mbed_error_printf("EP0 ack wait!!!!\r\n");
                if (set_addr) {
            ...
            case USB_EP0_STAGE_STATUSIN: {
                g_ep0_state = USB_EP0_STAGE_IDLE;
                //mbed_error_printf(USBD_DEBUG_LEVEL,"EP0 status in\r\n");
                goto setup;
            }
                break;
            case USB_EP0_STAGE_STATUSOUT: {
                g_ep0_state = USB_EP0_STAGE_IDLE;
                //mbed_error_printf(USBD_DEBUG_LEVEL,"EP0 status out\r\n");
                goto setup;
            }
            case USB_EP0_STAGE_IDLE: {
setup:
                //mbed_error_printf(USBD_DEBUG_LEVEL,"EP0 idle\r\n");
    ...
    for (ep_num = USB_MIN_EP_NUM; (ep_num < USB_MAX_EP_NUM) && (int_stat != 0); ep_num ++) {
        if (int_stat & EP(ep_num)) {
            ...
            //mbed_error_printf("tx interrupt ep_num is %d, txcsr is 0x%04x\r\n", ep_num, csr);
    ...
    //mbed_error_printf("bus interrupt int_stat is 0x%04x \r\n", int_stat);

    if (int_stat) {
        if (int_stat & USB_INTR_RESET) {
            //mbed_error_printf("reset \r\n");
            /* reset usb hardware */
            RDA_USB->INTRTXEN = BIT(0);
            RDA_USB->INTRRXEN = 0;
            //RDA_USB->INTREN = USB_INTR_RESET | USB_INTR_DISCONNECT;
...
void RDAendpointRead(uint8_t endpoint, uint8_t *buffer, uint32_t len) {
    ...
    for (i = 0; i < len; i++) {
        if ((i % 8) == 0)
            mbed_error_printf("\r\n");
        mbed_error_printf("%02x ", buffer[i]);
        if (i == len)
            mbed_error_printf("\r\n");
    }
#endif
...
void USBHAL::EP0setup(uint8_t *buffer) {
	//mbed_error_printf("EP0setup\r\n");
    RDAendpointRead(0, buffer, sizeof(SETUP_PACKET_T));
}

void USBHAL::EP0read(void) {
	//mbed_error_printf("EP0read\r\n");
}

EP_STATUS USBHAL::endpointWrite(uint8_t endpoint, uint8_t *data, uint32_t size) {
    uint32_t fifo_addr = (uint32_t)(RDA_USB->FIFOs + endpoint);
    uint16_t i = 0;
    //mbed_error_printf("endpointWrite enter endpoint is %d , size is %d \r\n", endpoint, size);
...
void USBHAL::EP0write(uint8_t *buffer, uint32_t size) {
    //mbed_error_printf("EP0write size is %d \r\n", size);
...
uint32_t USBHAL::EP0getReadResult(uint8_t *buffer) {
    ...
    //mbed_error_printf("EP0getReadResult count0 is %d\r\n", count0);
...
bool USBHAL::realiseEndpoint(uint8_t endpoint, uint32_t maxPacket, uint32_t options) {
    uint16_t csr;
	uint16_t int_rxe, int_txe;
    /* select endpoint */
    uint8_t index;

    index = RDA_USB->EPIDX;
    //mbed_error_printf("realise endpoint index is 0x%02x \r\n ", index);
    RDA_USB->EPIDX = endpoint;
    ...
    switch (endpoint) {
        ...
        case EPINT_OUT:
            ...
            //mbed_error_printf("realise endpoint int_rxe is 0x%02x , csr is 0x%04x\r\n ", int_rxe, RDA_USB->RXCSR);
...
EP_STATUS USBHAL::endpointReadResult(uint8_t endpoint, uint8_t *data, uint32_t *bytesRead) {
    ...
    //mbed_error_printf("endpointReadResult enter \r\n");
    /* select endpoint */
    RDA_USB->EPIDX = endpoint;
    ...
    //mbed_error_printf("endpointReadResult rxcsr is 0x%04x ,endpoint is %d\r\n", csr, endpoint);

    if (csr & USB_RXCSR_RXPKTRDY) {
        fifo_count = RDA_USB->RXCOUNT;
        //mbed_error_printf("endpointReadResult fifo_count is %d \r\n", fifo_count);
    ...
    //mbed_error_printf("endpointReadResult success \r\n");
...
EP_STATUS USBHAL::endpointWriteResult(uint8_t endpoint) {
    ...
    RDA_USB->EPIDX = endpoint;
    csr = RDA_USB->TXCSR;
    //mbed_error_printf("endpointWriteResult csr is 0x%04X\r\n", csr);
